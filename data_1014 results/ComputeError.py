...
def ComputeError(Controls, Observed_dir, Simu_dir):
...
if __name__ == "__main__":
    ...
    Control_points = sys.argv[4]
    if Control_points == "True": #Remove the error caused by control points  True->Remove False->Keep
        f = open("./Particle_index/control_thin.txt")
        iter_f = iter(f)
        Controls = []
        for line in iter_f:
            Controls.append(int(line))
    else:
        Controls = []
    ...
    NumpyErrors, InitialData = ComputeError(Controls, Observed_file, NoRegiSimu_file)
    ...
    NumpyErrors, InitialData = ComputeError(Controls, Observed_file, RegiSimu_file)
