...
int CreateClusters(Vec3* particles, const float* priority, int numParticles, std::vector<int>& outClusterOffsets,
                   std::vector<int>& outClusterIndices, std::vector<Vec3>& outClusterPositions, float radius,
...
int CreateLinks(const Vec3* particles, int numParticles, std::vector<int>& outSpringIndices,
                std::vector<float>& outSpringLengths, std::vector<float>& outSpringStiffness, float radius,
...
void CreateSkinning(const Vec3* vertices, int numVertices, const Vec3* clusters, int numClusters, float* outWeights,
                    int* outIndices, float falloff, float maxdist) {
...
void SampleMesh(const Vec3* vertices, int numVertices, const int* triangleIndices, int numIndices, float radius,
                float volumeSampling, float surfaceSampling, std::vector<Vec3>& outPositions) {
    Vec3 meshLower(FLT_MAX);
    Vec3 meshUpper(-FLT_MAX);

    for (int i = 0; i < numVertices; ++i) {
        meshLower = Min(meshLower, vertices[i]);  // the (MIN_x,MIN_y,MIN_z) of the loaded meshfile
        meshUpper = Max(meshUpper, vertices[i]);  // the (MAX_x,MAX_y,MAX_z) of the loaded meshfile
    }
    ...
    if (volumeSampling > 0.0f) {
        ...
        vector<uint32_t> voxels(maxDim * maxDim * maxDim);  // sampling voxels full x/y/z
        ...
        // with the mesh
        Vec3 meshOffset;
        meshOffset.x = 0.5f * (spacing - (edges.x - (dx - 1) * spacing));  // i.e., l/delta_'*delta - l
        meshOffset.y = 0.5f * (spacing - (edges.y - (dy - 1) * spacing));
        meshOffset.z = 0.5f * (spacing - (edges.z - (dz - 1) * spacing));
        meshLower -= meshOffset;

        Voxelize(vertices, numVertices, triangleIndices, numIndices, maxDim, maxDim, maxDim, &voxels[0], meshLower,
...
NvFlexExtAsset* NvFlexExtCreateSoftFromMesh(const float* vertices, int numVertices, const int* triangleIndices,
                                            int numtriangleIndices, float particleSpacing, float volumeSampling,
                                            float surfaceSampling, float clusterSpacing, float clusterRadius,
                                            float clusterStiffness, float linkRadius, float linkStiffness,
                                            float globalStiffness, float clusterPlasticThreshold,
                                            float clusterPlasticCreep) {
    // Switch to relative coordinates by computing the mean position of the vertices and subtracting the result from
    ...
    std::vector<Vec3> samples;
    // [SampleMesh Output] samples : the means of each clusters
    SampleMesh(relativeVertices, numVertices, triangleIndices, numtriangleIndices, particleSpacing, volumeSampling,
               surfaceSampling, samples);
    ...
    int numClusters = CreateClusters(&samples[0], &priority[0], int(samples.size()), clusterOffsets, clusterIndices,
                                     clusterPositions, clusterSpacing, clusterRadius);
    ...
    if (linkRadius > 0.0f) {
        ...
        int numLinks = CreateLinks(&samples[0], int(samples.size()), springIndices, springLengths, springStiffness,
...
void NvFlexExtCreateSoftMeshSkinning(const float* vertices, int numVertices, const float* bones, int numBones,
                                     float falloff, float maxDistance, float* skinningWeights, int* skinningIndices) {
    CreateSkinning((Vec3*)vertices, numVertices, (Vec3*)bones, numBones, skinningWeights, skinningIndices, falloff,
                   maxDistance);
}
